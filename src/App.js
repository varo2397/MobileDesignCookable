      ...
      <Fragment>
        {
          !isPortrait ?
            <span>
              {
                this.state.showNavigation && isMobile ? <MobileNavigation toggleNavigation={this.toggleMobileNavigation} /> : null
              }
              <div className="video-bg-wrapper" >

                <video loop muted autoPlay src={process.env.PUBLIC_URL + "/Beer_Pour_Videvo.mp4"} className="video-bg"></video>
              </div>
              <div className={`h-100 w-100 absolute ${isMobile && this.state.showNavigation ? 'show-navigation-mobile' : ''}`} onClick={() => this.toggleMobileNavigation()}>
